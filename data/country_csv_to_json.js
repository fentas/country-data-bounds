...
parser.on('finish', function(){
  ...
  _.each(['currencies', 'countryCallingCodes', 'languages', 'bounds'], function(key) {
    _.each(output, function (country) {
      country[key] = country[key] ? country[key].split(',').map(function(v) { if ( key == 'bounds' ) return parseFloat(v); return v; }) : [];
    });
  });
